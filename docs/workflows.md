...
graph TB
    ...
    Implement --> Deploy["<b>/deploy</b><br/>☁️ GCloud Agent<br/>creates IaC + deploys to GCP"]

    Delegate --> Deploy

    Deploy --> Done[("✅ Production-Ready<br/>Application on Google Cloud")]
...
graph TB
    ...
    Plan --> Deploy["<b>/deploy</b><br/>☁️ GCloud Agent<br/>creates IaC + deploys to GCP"]

    Deploy --> Done[("✅ Evolved Application<br/>Modernized and/or Extended<br/>on Google Cloud")]
...
6. **`/deploy`** - GCP Deployment
   ...
   - Deploys to Google Cloud using gcloud CLI and Terraform
